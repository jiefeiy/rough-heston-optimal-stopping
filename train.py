...
from solver import BSDEModelrHeston
import time
import torch.optim as optim
from torch.optim.lr_scheduler import LambdaLR, MultiStepLR


class DeepBSDEOSrHeston:
    """
    add phi feature to c_fun, where payoff = max(phi, 0)
    grad_fun as a function of s and v
    """
    def __init__(self, cfg, option, weights):
        self.cfg = cfg
    ...
    def train(self):
        ...
        dw_train, s_train, v_train = self.option.sample(num_samples)
        ...
        v_train = v_train.to(self.device)
        ...
        for k in range(self.num_time_step - 1, 0, -1):
            if k == self.num_time_step - 1:
                model = BSDEModelrHeston(self.cfg, self.option, k, self.weights)  # input [phi, x, v]
            else:
                model = BSDEModelrHeston(self.cfg, self.option, k, self.weights, init_c=c_fun[k+1], init_grad=g_fun[k+1])
            model.to(self.device)
            optimizer = optim.Adam(model.parameters(), lr=self.lr)
            if k == self.num_time_step - 1:
                scheduler = LambdaLR(optimizer, lr_lambda=lambda n: 1e-3 * 1e-4 ** (max((n - 100) / 1000, 0)))
                num_epochs = self.num_epochs * 2
            else:
                scheduler = LambdaLR(optimizer, lr_lambda=lambda n: 1e-3 * 1e-4 ** (max((n - 100) / 500, 0)))
            ...
            for epoch in range(num_epochs):
                for step in range(self.num_iterations):
                    ...
                    ss_train_batch_k = torch.cat((phi_matrix[idx_batch, k].reshape((self.batch_size, 1)),
                                                  s_train[idx_batch, k].unsqueeze(1), v_train[idx_batch, k].unsqueeze(1)), dim=1)
            ...
            with torch.no_grad():
                model.eval()
                ss_train_k = torch.cat((phi_matrix[:, k].reshape((num_samples, 1)), s_train[:, k].unsqueeze(1), v_train[:, k].unsqueeze(1)), dim=1)
